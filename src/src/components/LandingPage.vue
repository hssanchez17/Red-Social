  ...
  <b-row >
    ...
    <b-col id="column2">
      ...
      <b-form @submit.prevent="register(userRegister)">
        ...
        <b-form-group>
          <b-form-input placeholder="Ingrese su Email" id="emailRegister" v-model.trim="$v.userRegister.email.$model" :class="{'is-invalid':$v.userRegister.email.$error,'is-valid':!$v.userRegister.email.$invalid}" v-on:keyup="submitEmail=false"></b-form-input>
          <span  class="valid-feedback" >Su email es valido</span>
          <span class="invalid-feedback" v-if="$v.userRegister.email.required && !submitEmail " >Ingrese un email valido</span> 
